...
SERVER_VERSION = "2026.05.12-order-split-1"
...
def backtest_score(df, index, orb_high_value, orb_low_value, gap_day=False):
    ...
    if row["EMA9"] > row["EMA21"]:
        ce_score += 1
    if not gap_day and row["close"] > (orb_high_value + ORB_BREAK_BUFFER_POINTS):
        ce_score += 1
    if has_strong_two_candle_momentum(prev, row, "CE"):
        ce_score += 1
    if row["close"] < row["VWAP"]:
        pe_score += 1
    ...
    if row["EMA9"] < row["EMA21"]:
        pe_score += 1
    if not gap_day and row["close"] < (orb_low_value - ORB_BREAK_BUFFER_POINTS):
        pe_score += 1
    if has_strong_two_candle_momentum(prev, row, "PE"):
        pe_score += 1
...
def backtest_signal(df, index, orb_high_value, orb_low_value, gap_day=False):
    ...
    ce_orb_ok = (not gap_day) and ce_breakout
    pe_orb_ok = (not gap_day) and pe_breakout
    ...
    ce_half_core = row["close"] > row["VWAP"] and ce_supertrend and ce_momentum and row["EMA9"] > row["EMA21"] and gap_down_ce_sustain
    pe_half_core = row["close"] < row["VWAP"] and pe_supertrend and pe_momentum and row["EMA9"] < row["EMA21"] and gap_up_pe_sustain
    ce_full_core = ce_half_core and ce_orb_ok and (not gap_day) and row["close"] > prev["high"]
    pe_full_core = pe_half_core and pe_orb_ok and (not gap_day) and row["close"] < prev["low"]

    if ce_score > pe_score and ce_half_core:
        timing_ok, _ = entry_timing_guard(df.iloc[: index + 1], "CE", row["close"], row["VWAP"], row["EMA9"])
        if not timing_ok:
            return None, "NONE", ce_score
        trade_type = "FULL" if ce_full_core else "HALF"
        return "CE", trade_type, ce_score
    if pe_score > ce_score and pe_half_core:
        timing_ok, _ = entry_timing_guard(df.iloc[: index + 1], "PE", row["close"], row["VWAP"], row["EMA9"])
        if not timing_ok:
            return None, "NONE", pe_score
        trade_type = "FULL" if pe_full_core else "HALF"
        return "PE", trade_type, pe_score
    return None, "NONE", max(ce_score, pe_score)
...
SERVER_VERSION = "2026.05.14-fast-status-telegram-1"
