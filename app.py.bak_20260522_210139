    ...
    return {
        ...
        "release_notes": config.get(
            "mobile_app_release_notes",
                "Strategy updated: realistic 1-minute option OHLC backtest, tick-path simulation, and profit lock ladder 15/20/25/30/35/45/50%.",
        ),
...
SERVER_VERSION = "2026.05.22-live-ready-switch-1"
MOBILE_APP_VERSION = "1.0.0-build13"
...
SERVER_VERSION = "2026.05.22-weighted-gemini-2"
...
def option_chain_ai_analysis(signal, option, premium, spot_price):
    ...
    try:
        ...
        ok = bool((base or {}).get("ok", True)) and bool(decision.get("take_trade", False))
        reason = (
            f"Weighted {weighted_score}/100 | Gemini {decision.get('confidence', 0)}% | "
            f"Regime {last_intelligence_context.get('market_regime')} | "
            f"Chain {chain.get('reason', 'neutral')} | {decision.get('reason', '')}"
        )
